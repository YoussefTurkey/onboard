    ...
    <>
        <section className={`${styles.blogs} container mx-auto my-40 flex flex-col items-center`} id='blogs'>
            ...
            <div className={`${styles.blogs} grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[10px] sm:gap-[10px] md:gap-[30px] my-10 md:mx-[85px] justify-items-center`}>
            {
                posts.map( post => {
                    return(
                        <Link href={post.to ? post.to : ''} key={post.id} className={`${styles.blog} ${post.style ? styles.expand : ''} flex flex-col items-start bg-bgHover border border-bgHover hover:border-primary rounded-3xl w-[100%] my-3 sm:my-0 transition-all`}>
                            <Image src={post.src} alt={post.title} loading='lazy' width='300' height='300' className='w-[100%] h-[150px] sm:h-[300px] rounded-tr-3xl rounded-tl-3xl border-b-[8px] border-primary object-cover' />
                            <div className="p-5">
