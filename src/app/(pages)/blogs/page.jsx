...
const data = {
    sectionData: {
      ...
      post: [
        ...
        {id: 3,src: '/images/photos/post_3.webp', title: 'Visual Identity Branding', date: '08 May 2024', category:'Graphic Desing', to: '/'},
      ]
    }
}
  ...
  return(
    <>
        <section className={`${styles.blogs} container mx-auto my-40 flex flex-col items-center`} id='blogs'>
            ...
            <div className={`${styles.blogs} grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-[10px] sm:gap-[20px] md:gap-[30px] my-10 md:mx-[85px] justify-items-center`}>
            {
                posts.map( post => {
                    return(
                        <Link href={post.to ? post.to : ''} key={post.id} className={`${styles.card} flex flex-col items-start gap-[15px] bg-bgHover border border-bgHover hover:border-primary rounded-3xl w-[250px] md:w-[100%] transition-all`}>
                            <Image src={post.src} alt={post.title} loading='lazy' width='300' height='300' className='w-[100%] h-[300px] rounded-tr-3xl rounded-tl-3xl border-b-[8px] border-primary object-cover' />
                            <div className="p-5">
