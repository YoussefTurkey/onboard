    ...
    <>
        <section className={`${styles.contact} mx-auto container my-40`} id='contact'>
            ...
            <div className={`${styles.cards} grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[10px] sm:gap-[20px] md:gap-[30px]  my-10 md:mx-[85px] justify-items-center`}>
            {
                cards.map( skill => {
                    return(
                        <Link href={skill.to ? skill.to : ''} target='_blank' key={skill.id} className={`${styles.card} ${skill.style ? styles.expand : ''} flex flex-col items-center gap-[15px] border border-bgHover hover:border-primary rounded-lg p-[40px] w-[80%] md:w-[100%] transition-all`}>
